...
// Lead-agent state — when the lead agent is running, this holds the instance
// so the resume/skip IPC handlers can call into it. Null when no lead run
// is active or when running the legacy (non-lead) execution path.
let activeLeadAgent: LeadAgentService | null = null;
let lastBuildDir: string | null = null;
// Counter for DEMO_MODE lead-agent providers — each worker gets a fresh
// DemoLeadProvider, and we use this counter to pick the milestone name.
// Reset to 0 each time a new lead run starts.
let demoProviderCount = 0;
...
/**
 * DemoLeadProvider — a fake IConstructAIProvider for DEMO_MODE execution.
 *
 * In DEMO_MODE, refinement and planning are short-circuited to return
 * deterministic fake data. But execution goes through LeadAgentService,
 * which dispatches real AgentLoop instances that call provider.chat().
 * Without a real API key, that would fail.
 *
 * This provider yields a realistic AgentLoopEvent stream per call:
 *   - Round 1: tool_start + tool_end for write_file (writes a real file
 *     to disk via the actual staging layer), then done(tool_use).
 *   - Round 2: token + done(end_turn) → worker exits.
 *
 * This means DEMO_MODE execution writes REAL FILES to the build dir,
 * exercises the REAL staging/approval/verification mechanism, and lets
 * the lead/worker UI be screenshot-tested end-to-end without an API key.
 *
 * The provider is stateful per-instance — call 1 does the write, call 2
 * does the summary. The lead agent creates a FRESH provider per worker,
 * so each milestone gets call 1 → call 2 from a fresh state.
 */
class DemoLeadProvider implements IConstructAIProvider {
    readonly providerType: 'cloud' = 'cloud';
    private _callCount = 0;
    constructor(private _milestoneName: string) {}

    async *chat(): AsyncIterable<AgentLoopEvent> {
        this._callCount++;
        if (this._callCount === 1) {
            const fileName = this._milestoneName.replace(/[^a-zA-Z0-9]/g, '-').toLowerCase() + '.txt';
            const content = '# Output of milestone: ' + this._milestoneName + '\n' +
                'Generated by DemoLeadProvider at ' + new Date().toISOString() + '\n' +
                'This is a DEMO_MODE file — real file write through the real staging layer.\n';
            yield { type: 'tool_start', toolId: 'demo-tool-1', toolName: 'write_file' } as AgentLoopEvent;
            yield {
                type: 'tool_end',
                toolId: 'demo-tool-1',
                toolName: 'write_file',
                toolInput: { path: fileName, content },
            } as AgentLoopEvent;
            yield { type: 'done', stopReason: 'tool_use' } as AgentLoopEvent;
        } else {
            yield { type: 'token', text: 'Milestone "' + this._milestoneName + '" complete. Wrote one demo file.' } as AgentLoopEvent;
            yield { type: 'done', stopReason: 'end_turn' } as AgentLoopEvent;
        }
    }
...
ipcMain.handle('kovix:exec:abort', async (): Promise<{ ok: boolean }> => {
    ...
    // If the lead agent is paused, unblock it too.
    if (activeLeadAgent) {
        activeLeadAgent.skipPausedMilestone();
    }
    return { ok: true };
});

// ----------------------------------------------------------------------
// LEAD AGENT ORCHESTRATION (sequential milestone delegation)
// ----------------------------------------------------------------------

/**
 * Send a lead-agent event to the renderer if the window is alive.
 * Lead events are forwarded as-is — the renderer's Execute screen
 * renders the lead/worker visual structure from these.
 */
function sendLeadEvent(event: LeadAgentEvent): void {
    try {
        if (mainWindow && !mainWindow.isDestroyed()) {
            mainWindow.webContents.send('kovix:exec:lead-event', event);
        }
    } catch (err) {
        console.error('[kovix:exec:lead] failed to send event to renderer:', err);
    }
}

/**
 * Start the lead agent. The lead iterates through the approved plan's
 * milestones sequentially, dispatching a FRESH AgentLoop instance per
 * milestone. Each worker runs to completion (or pause per pre-flight
 * config) and returns a structured WorkerReport. The lead reviews each
 * report before dispatching the next milestone — if a worker's
 * verification failed, the lead STOPS (per Part 1 principles).
 *
 * This REUSES the existing staging/approval/verification mechanism —
 * the lead is a coordination layer on top of AgentLoop, not a second
 * execution path.
 *
 * The IPC handler returns { ok: true } immediately after the lead
 * finishes (or fails). All progress comes via 'kovix:exec:lead-event'
 * events. The renderer listens for these and renders the lead/worker
 * structure on the Execute screen.
 */
ipcMain.handle('kovix:exec:start-lead', async (): Promise<{ ok: boolean; error?: string }> => {
    if (session.stage !== 'executing') {
        return { ok: false, error: 'Not in executing stage. Confirm preflight first.' };
    }
    if (session.milestones.length === 0) {
    ...
    const cfg = await getProviderConfig();
    // Per-build workspace: <workspaceDir>/build-<timestamp>/
    const fs = await import('node:fs/promises');
    const workspaceBase = await getEffectiveWorkspaceDir();
    const buildDir = path.join(workspaceBase, 'build-' + Date.now());
    await fs.mkdir(buildDir, { recursive: true });
    lastBuildDir = buildDir;
    console.log('[kovix:exec:lead] build workspace:', buildDir);
    ...
    const lead = new LeadAgentService({
        createProvider: () => {
            // FRESH provider per worker.
            //
            // In DEMO_MODE, return a DemoLeadProvider — a fake that yields
            // a realistic event stream and writes a real file to disk via
            // the actual staging layer. This lets the full UI flow be
            // exercised end-to-end without an API key.
            //
            // In production mode, createProvider(cfg) builds a real provider
            // (Anthropic / OpenRouter / Ollama / etc.) using the user's
            // stored settings.
            if (DEMO_MODE) {
                // Pick the next pending milestone's name for the demo file.
                // The lead creates providers in milestone order, so we can
                // use the count of providers created so far as an index.
                const idx = demoProviderCount++;
                const milestone = session.milestones[idx] || session.milestones[0];
                return new DemoLeadProvider(milestone ? milestone.name : 'demo') as unknown as IConstructAIProvider;
            }
        ...
        spec: session.spec,
        buildDir,
        pauseMode: session.preflight.pauseMode,
        customPauseIds: session.preflight.customPauseIds,
        approveWrite: async (filePath: string, proposedContent: string, _existing: string | null) => {
            approvalFired = true;
            console.log('[kovix:exec:lead] APPROVAL CALLBACK: ' + filePath + ' (' + proposedContent.length + ' chars)');
            // Verify staging: file should NOT exist on disk yet.
            try {
                await fs.readFile(path.join(buildDir, filePath), 'utf8');
                fileWrittenBeforeApproval = true;
                console.error('[kovix:exec:lead] VIOLATION: file written before approval!');
            } catch (err: unknown) {
                if ((err as NodeJS.ErrnoException).code === 'ENOENT') {
                    console.log('[kovix:exec:lead] confirmed: file NOT on disk before approval (staging works)');
                }
            }
            return true; // auto-approve (the staging layer already blocked the write)
        },
        approveInterpreterCommand: async (cmd: string) => {
            console.log('[kovix:exec:lead] AUTO-APPROVING interpreter command: ' + cmd);
            return true;
        },
        log: (msg: string) => console.log('[lead] ' + msg),
    });
    activeLeadAgent = lead;
    demoProviderCount = 0; // reset for this run
    executionAbortController = new AbortController();

    try {
        const stream = lead.run(executionAbortController.signal);

        for await (const event of stream as AsyncIterable<LeadAgentEvent>) {
            if (executionAbortController?.signal.aborted) {
                sendLeadEvent({ type: 'lead_error', text: '[STOP] Aborted by user', recoverable: false });
                break;
            }
            ...
            // Map lead events onto the existing session.execution state
            // so the existing Execute screen rendering (milestone cards
            // with status badges) keeps working in parallel.
            if (event.type === 'lead_reviewing' || event.type === 'worker_started') {
                const ms = session.execution.milestoneStates.find(s => s.milestoneId === event.milestone.id);
                if (ms && event.type === 'worker_started') {
                    ms.status = 'running';
                    ...
                    try {
                        ...
                        console.error('[kovix:exec:lead] failed to send state update:', err);
                    }
                }
            } else if (event.type === 'worker_completed') {
                const r: WorkerReport = event.report;
                const ms = session.execution.milestoneStates.find(s => s.milestoneId === r.milestoneId);
                if (ms) {
                    ms.status = r.errored ? 'failed' : 'completed';
                    ms.endedAt = Date.now();
                    ms.verificationPassed = r.verificationPassed;
                    ms.verificationOutput = r.verificationOutput;
                    ms.events = r.eventLog.slice(-200).map(e => ({
                        type: e.type,
                        text: ('text' in e ? e.text : '') || ('toolName' in e ? e.toolName : '') || ('filePath' in e ? e.filePath : ''),
                        ts: Date.now(),
                    }));
                    try {
                        if (mainWindow && !mainWindow.isDestroyed()) {
            ...
            } else if (event.type === 'lead_paused') {
                const ms = session.execution.milestoneStates.find(s => s.milestoneId === event.milestone.id);
                if (ms) { ms.status = 'paused'; }
                sendExecEvent({
                    type: 'milestone_paused',
                    milestone: {
                        ...
                        isMajor: event.milestone.isMajor,
                        stepIndices: [event.milestoneIndex],
                        completed: false,
                    },
                } as AgentLoopEvent);
            } else if (event.type === 'lead_resumed') {
                sendExecEvent({
            ...
            } else if (event.type === 'lead_blocked') {
                // A worker failed verification — surface as a real error.
                sendExecEvent({
                    type: 'error',
                    text: event.reason,
                    recoverable: true,
                });
            } else if (event.type === 'lead_complete') {
                session.stage = 'done';
                session.execution.endedAt = Date.now();
                session.execution.currentMilestoneId = null;
                try {
                    ...
                    console.error('[kovix:exec:lead] failed to send complete event:', err);
                }
                executionAbortController = null;
                activeLeadAgent = null;
                return { ok: true };
            } else if (event.type === 'lead_error') {
                sendExecEvent({ type: 'error', text: event.text, recoverable: event.recoverable });
                if (!event.recoverable) {
                    executionAbortController = null;
                    activeLeadAgent = null;
                    return { ok: false, error: event.text };
                }
            }
        }

        // Stream ended without a lead_complete (probably aborted).
        session.execution.endedAt = Date.now();
        executionAbortController = null;
        activeLeadAgent = null;
        return { ok: true };
        ...
        const msg = err instanceof Error ? err.message : String(err);
        console.error('[kovix:exec:start-lead] error:', msg);
        session.execution.endedAt = Date.now();
        executionAbortController = null;
        activeLeadAgent = null;
        sendLeadEvent({ type: 'lead_error', text: msg, recoverable: false });
        return { ok: false, error: msg };
    }
});

/**
 * Resume from a lead-agent pause. Only valid when the lead is paused.
 */
ipcMain.handle('kovix:exec:lead-resume', async (): Promise<{ ok: boolean; error?: string }> => {
    if (!activeLeadAgent) {
        return { ok: false, error: 'Lead agent not active.' };
    }
    activeLeadAgent.resumeFromPause();
    return { ok: true };
});

/**
 * Skip the current paused milestone in the lead agent.
 */
ipcMain.handle('kovix:exec:lead-skip', async (): Promise<{ ok: boolean; error?: string }> => {
    if (!activeLeadAgent) {
        return { ok: false, error: 'Lead agent not active.' };
    }
    activeLeadAgent.skipPausedMilestone();
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  /* ---- LEAD / WORKER PANELS (sequential delegation visual structure) ---- */
  .lead-panel, .worker-panel {
    border: 1px solid var(--border);
  ...
  .lead-panel-header, .worker-panel-header {
    display: flex;
    align-items: center;
    gap: 8px;
    font-weight: 600;
  ...
  .lead-panel-icon, .worker-panel-icon {
    font-size: 14px;
  }
  .lead-panel-icon { color: var(--accent); }
  .worker-panel-icon { color: #10B981; }
  .lead-panel-detail, .worker-panel-detail {
    margin-top: 6px;
    color: var(--text-2);
    font-size: 12px;
    line-height: 1.5;
  ...
  .lead-panel.is-blocked {
    border-left-color: var(--danger);
    background: var(--danger-soft, rgba(239, 68, 68, 0.08));
  }
  .worker-panel.is-running .worker-panel-icon {
  ...
  <main>
    <div class="container">
      ...
      <section id="state-execute" class="execute-screen hidden">
        ...
        <div id="lead-panel" class="lead-panel hidden">
          <div class="lead-panel-header">
            <span class="lead-panel-icon">&#9881;</span>
            <span id="lead-panel-status">Lead Agent: initializing&hellip;</span>
          </div>
          <div id="lead-panel-detail" class="lead-panel-detail"></div>
        </div>
        <div id="worker-panel" class="worker-panel hidden">
          <div class="worker-panel-header">
            <span class="worker-panel-icon">&#9881;</span>
            <span id="worker-panel-status">Worker: idle</span>
          </div>
          <div id="worker-panel-detail" class="worker-panel-detail"></div>
        </div>
...
(function() {
  ...
  const els = {
    ...
    leadPanelStatus: document.getElementById('lead-panel-status'),
    leadPanelDetail: document.getElementById('lead-panel-detail'),
    workerPanel: document.getElementById('worker-panel'),
    workerPanelStatus: document.getElementById('worker-panel-status'),
    workerPanelDetail: document.getElementById('worker-panel-detail'),
    // Done
  ...
  els.preflightConfirm.addEventListener('click', async () => {
    ...
    try {
      ...
      renderExecuteScreen();
      showState('execute');
      // Start the LEAD AGENT (sequential milestone delegation). Events flow
      // via onLeadEvent (lead_reviewing, worker_started, worker_completed,
      // lead_blocked, lead_complete) AND via onEvent/onStateUpdate for
      // legacy milestone-card rendering. The lead agent dispatches a FRESH
      // AgentLoop per milestone — see src/agent/leadAgent/leadAgentService.ts.
      els.leadPanel.classList.remove('hidden');
      els.leadPanelStatus.textContent = 'Lead Agent: starting\u2026';
      els.leadPanelDetail.textContent = '';
      els.workerPanel.classList.add('hidden');
      await kovixAPI.exec.startLead();
    } catch (err) {
      showError(err.message || String(err));
    } finally {
  ...
  // ---- LEAD AGENT EVENT LISTENER ----
  // Updates the lead/worker panels on the Execute screen based on
  // LeadAgentEvent stream. This is the visual structure that shows
  // the lead delegating to fresh workers, milestone by milestone.
  kovixAPI.exec.onLeadEvent((event) => {
    if (!event || typeof event !== 'object' || !('type' in event)) return;
    const ev = event;
    if (ev.type === 'lead_started') {
      els.leadPanel.classList.remove('hidden');
      els.leadPanel.classList.remove('is-blocked');
      els.leadPanelStatus.textContent = 'Lead Agent: planning delegation for ' + ev.totalMilestones + ' milestone' + (ev.totalMilestones === 1 ? '' : 's');
      els.leadPanelDetail.textContent = ev.milestones.map((m, i) => (i + 1) + '. ' + m.name).join('\\n');
      els.workerPanel.classList.add('hidden');
      els.workerPanel.classList.remove('is-running');
    } else if (ev.type === 'lead_reviewing') {
      els.leadPanel.classList.remove('hidden');
      els.leadPanel.classList.remove('is-blocked');
      els.leadPanelStatus.textContent = 'Lead Agent: reviewing milestone ' + (ev.milestoneIndex + 1) + ' of ' + ev.totalMilestones;
      els.leadPanelDetail.textContent = 'Next: ' + ev.milestone.name + '\\n' + ev.milestone.description;
    } else if (ev.type === 'worker_started') {
      els.workerPanel.classList.remove('hidden');
      els.workerPanel.classList.add('is-running');
      els.workerPanelStatus.textContent = 'Worker: milestone ' + (ev.milestoneIndex + 1) + ' \\u2014 ' + ev.milestone.name;
      els.workerPanelDetail.textContent = 'Fresh agent loop dispatched. Scope: ' + ev.milestone.description;
    } else if (ev.type === 'worker_event') {
      // Passthrough of an AgentLoopEvent from the worker. We only surface
      // the interesting ones in the worker panel detail; milestone cards
      // (rendered from onStateUpdate) carry the full state.
      const inner = ev.event;
      if (!inner || typeof inner !== 'object' || !('type' in inner)) return;
      if (inner.type === 'file_written') {
        const cur = els.workerPanelDetail.textContent || '';
        els.workerPanelDetail.textContent = cur + '\\n[wrote] ' + inner.filePath;
        // Auto-scroll to bottom
        els.workerPanelDetail.scrollTop = els.workerPanelDetail.scrollHeight;
      } else if (inner.type === 'verification_result') {
        const cur = els.workerPanelDetail.textContent || '';
        const tag = inner.unverified ? 'unverified' : (inner.passed ? 'passed' : 'FAILED');
        els.workerPanelDetail.textContent = cur + '\\n[verification ' + tag + ']';
      ...
      const filesLine = r.filesChanged.length > 0
        ? 'Files: ' + r.filesChanged.join(', ')
        : 'No files written.';
      els.workerPanelDetail.textContent =
        'Status: ' + status + ' (' + r.durationMs + 'ms)\\n' +
        filesLine + '\\n' +
        'Summary: ' + (r.summary || '(none)');
    } else if (ev.type === 'lead_blocked') {
      els.leadPanel.classList.add('is-blocked');
      els.leadPanelStatus.textContent = 'Lead Agent: BLOCKED at milestone ' + (ev.milestoneIndex + 1) + ' of ' + ev.totalMilestones;
      els.leadPanelDetail.textContent = ev.reason + '\\n\\nThe lead has stopped per Part 1 principles \\u2014 it will not proceed on a broken foundation.';
      els.workerPanel.classList.remove('is-running');
    } else if (ev.type === 'lead_paused') {
      els.leadPanelStatus.textContent = 'Lead Agent: paused at milestone ' + (ev.milestoneIndex + 1) + ' of ' + ev.totalMilestones;
      els.leadPanelDetail.textContent = 'Milestone: ' + ev.milestone.name + '\\nPre-flight config says pause here. Resume or skip.';
    } else if (ev.type === 'lead_resumed') {
      els.leadPanelStatus.textContent = 'Lead Agent: resuming (action: ' + ev.action + ')';
    } else if (ev.type === 'lead_complete') {
      els.leadPanel.classList.remove('is-blocked');
      els.leadPanelStatus.textContent = 'Lead Agent: complete \\u2014 all ' + ev.reports.length + ' milestone(s) done';
      const lines = ev.reports.map(function (r) {
        const status = r.errored ? 'ERRORED' : (r.verificationUnverified ? 'unverified' : (r.verificationPassed ? 'verified' : 'FAILED'));
        return '[' + r.milestoneId + '] ' + r.milestoneName + ' \\u2014 ' + status + ' (' + r.filesChanged.length + ' file(s))';
      });
      els.leadPanelDetail.textContent = lines.join('\\n') + '\\n\\nBuild dir: ' + ev.buildDir;
      els.workerPanel.classList.add('hidden');
      els.workerPanel.classList.remove('is-running');
    } else if (ev.type === 'lead_error') {
      els.leadPanel.classList.add('is-blocked');
      els.leadPanelStatus.textContent = 'Lead Agent: ERROR';
      els.leadPanelDetail.textContent = ev.text;
    }
  });
  ...
  els.execResume.addEventListener('click', async () => {
    els.executePauseBanner.classList.add('hidden');
    // Try lead-resume first; if no lead is active, fall back to legacy resume.
    // Both IPC handlers return { ok: false, error } when not applicable, so
    // we can call both safely.
    const leadResult = await kovixAPI.exec.leadResume();
    if (!leadResult.ok) {
  ...
  els.execSkip.addEventListener('click', async () => {
    els.executePauseBanner.classList.add('hidden');
    const leadResult = await kovixAPI.exec.leadSkip();
    if (!leadResult.ok) {
